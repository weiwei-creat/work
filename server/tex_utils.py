...
def export_layout_to_mesh_dict_list_tree_search_with_object_id(layout: FloorPlan, object_id: str):
    import os
    import pickle
    ...
    while True:
        obj_mesh = get_object_mesh(target_object.source, target_object.source_id, layout.id)
        if obj_mesh is not None:
            # Apply transforms to the object mesh
            transformed_mesh, final_transform = apply_object_transform_return_transform(obj_mesh, target_object)
            texture_info = get_object_mesh_texture(target_object.source, target_object.source_id, layout.id)

            mesh_info_dict[target_object.id] = {
        ...
        parent_id = target_object.place_id
        if parent_id == 'floor' or parent_id == 'wall':
            break
        else:
            new_target_object = None
            room_objects = target_room.objects
            for obj in room_objects:
                if obj.id == parent_id:
                    new_target_object = obj
                    break
...
def export_layout_to_mesh_dict_object_id(layout: FloorPlan, object_id: str):
    import os
    import pickle
    ...
    if obj_mesh is not None:
        # Apply transforms to the object mesh
        transformed_mesh = apply_object_transform(obj_mesh, target_object)
        texture_info = get_object_mesh_texture(target_object.source, target_object.source_id, layout.id)
...
def export_layout_all_objects_mesh_dict_list_no_object_transform(layout: FloorPlan):
    """
    Export a FloorPlan object to a mesh dict list.
    Includes all object meshes with their transforms.
    ...
    for room in layout.rooms:
        # Create object meshes with transforms
        for obj in room.objects:
            if obj.source_id in source_id_list:
                continue
            source_id_list.append(obj.source_id)
            obj_mesh = get_object_mesh(obj.source, obj.source_id, layout.id)
            if obj_mesh is not None:
                # Apply transforms to the object mesh
                # transformed_mesh = apply_object_transform(obj_mesh, obj)
